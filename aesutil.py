'''General AES cipher utilites in python.'''
import secrets
import argparse
import base64
...
def encrypt(data: str, key: str) -> str:
    ...
    return (base64.b64encode(bytes(nonce)+b'|'+bytes(tag)+b'|'+bytes(ciphertext))).decode()

def decrypt(data: str, key: str) -> str:
    '''Decrypt data using key.'''
    key_b = base64.b64decode(key)
    nonce, tag, ciphertext = (base64.b64decode(data.encode())).split(b'|', maxsplit=2)
    cipher = AES.new(key_b, AES.MODE_EAX, nonce=nonce)
    plaintext = cipher.decrypt_and_verify(ciphertext, tag) # type: ignore
...
if __name__ == '__main__':
    ...
    parser_genkey.add_argument('-l', '--length',
          type=int, default=256, help='Length of the key.', choices=[128,192,256,512])

    # Encrypt data using key
    parser_encrypt = subparser.add_parser('enc', help='Encrypt data using a key.')
    parser_encrypt.add_argument('data',
        type=str, help='Data to encrypt.')
    key_encr_grp = parser_encrypt.add_mutually_exclusive_group()
    key_encr_grp.add_argument('-k', '--key',
        type=str, help='Key to use for encryption.')
    key_encr_grp.add_argument('-ke', '--keyenv',
        help='Use evnironmental value AES_KEY to encrypt.', action='store_true')

    # Decrypt data using key
    parser_decrypt = subparser.add_parser('dec', help='Decrypt data using a key.')
    parser_decrypt.add_argument('data',
        type=str, help='Data to decrypt.')
    key_decr_grp = parser_decrypt.add_mutually_exclusive_group()
    key_decr_grp.add_argument('-k', '--key',
        type=str, help='Key to use for decryption.')
    key_decr_grp.add_argument('-ke', '--keyenv',
        help='Use evnironmental value AES_KEY to decrypt.', action='store_true')
